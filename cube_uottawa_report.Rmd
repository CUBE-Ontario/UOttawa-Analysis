...
coefficients_table <- function(model) {
  model |> 
...
incidence_ratio_table <- function(model) {
  model |> 
    broom::tidy(exponentiate = T, conf.int = T, conf.level = 0.95) |> 
  mutate(across(where(is.double), ~round(., 4))) |> 
  kableExtra::kable(
    format = 'pipe',
    caption = "Poisson model with interactions: Incidence ratios & 95% CI"
    )
}
...
Backwards selection (and stepwise) arrived at the model with `swab_pcr`, `wifi`, `ottawa_ww`, and the interaction between `ottawa_ww` & `swab_pcr`.  
...
# do backward elim
mod_pois_intrx_stepwise <- step(mod_pois_intrx, trace = F)

# stepwise_poisson <- step(mod_pois_interactions, trace = F, direction = 'both')
stopifnot(mod_pois_intrx_stepwise$converged)

# standardized coefs and 95% CI
mod_pois_intrx_stepwise |> coefficients_table()

# incidence ratios
mod_pois_intrx_stepwise |> incidence_ratio_table()


# use type III SS tests since have significant interactions 
mod_pois_intrx_stepwise |> anova_table(type = 3, caption = "Type III ANOVA")

# test model assumption of equidispersion
# p > 0.05 means no significant overdispersion
AER::dispersiontest(mod_pois_intrx_stepwise)
```

<details>
<summary>***Backward selection model summary and diagnostics***</summary>
```{r}
summary(mod_pois_intrx)
summary(mod_pois_intrx_stepwise)
hist(mod_pois_intrx_stepwise$residuals)
# plot(mod_pois_interactions_stepwise)
rm(mod_pois_intrx)
```
</details>
...
fig1_uo_ww <-
  ggplot(uottawa_ww_biweekly,
         aes(biweek, signal)) +
  geom_smooth(span = 0.4, alpha = 0.2, linewidth = 0.5,
...
```{r}
table1 <- mod_pois_main_eff_stepwise |> incidence_ratio_table()
table1
...
**Figure S1.** Daily university wastewater sampling.
...
ggsave('fig/figure_s1.png', figure_s1, device = 'png', dpi = 300, height = 4, width = 7)

mapview::mapshot(figure_sites_map, file = 'fig/map.pdf',
                 remove_controls = T)
```
...
**Figure S2: Map of swab and waste-water sampling sites.** Pink markers represent waste-water collection sites; purple markers represent buildings where surface testing was performed. Only one building, Tabaret Hall, had both surface and waste-water testing. The locations of two waste-water sampling sites could not be ascertained and are not displayed on the map.
...
rs_abstract <- rs_data |> 
  glue::glue_data(
    "Over the 32-week study period, we collected {N_SWABS} swabs at six university buildings, including two buildings where wastewater surveillance was currently ongoing. 
Overall, {POS}% of swabs were PCR-positive for SARS-CoV-2, with individual buildings ranging between {POS_MIN}% and {POS_MAX}%.
...
```{r written rs, results='asis'}
result_p1 <- 
  lst(
    corr_swab_uniww = corr_ci |> filter(terms == 'SwPCR-UnvWW'),
    COR = corr_swab_uniww |> pluck('r') |> round(2), # 0.7
    corr_r_lo = corr_swab_uniww |> pluck('lower') |> round(2),
    corr_r_hi = corr_swab_uniww |> pluck('upper') |> round(2),
    COR_CI = str_glue('{corr_r_lo} - {corr_r_hi}')
  ) |> 
  glue::glue_data(
    "
#### Detection of SARS-CoV-2 from floor swabs and wastewater across campus

From September 2021 to April 2022, we conducted environmental surveillance of SARS-CoV-2 across the University of Ottawa campus using both built environment swabbing and wastewater testing. Floor swabs were collected twice weekly from six buildings on the main campus, including a residence hall, the main library, and lecture and administration buildings. The floors of two sites within each building were sampled, a high-traffic site close to the main entrance, and a low-traffic site at a more remote location within the building. Wastewater was collected from six sites across the main campus over the study period, however only one site (TBT) had any concurrent surface swab observations (Fig. S1). SARS-CoV-2 RNA was detected from built environment and wastewater samples according to established protocols (see Methods) (refs).

 	SARS-CoV-2 prevalence was low during the fall term (September to December 2021) according to both aggregate floor swab positivity and normalized wastewater signal (Fig. 1). However, the winter term (January to April 2022) was characterized by two spikes in environmental SARS-CoV-2 signal in January and late March with an intervening trough. This pattern was present in both the built environment and wastewater results and was driven by a subset of the individual sites sampled, specifically 90U, LPR, and MRT buildings (Fig. 3). We found a significant correlation between swab positivity and wastewater signal (Spearman r = {COR}, 95% CI: {COR_CI}). The signal increases occurred after the local introduction of the Omicron variant and correspond with two city-wide peaks in COVID-19 prevalence reported in Ottawa wastewater and case data.
")
...
Non-invasive approaches to monitoring SARS-CoV-2 levels such as environmental sampling are valuable when systematic testing of individuals is unfeasible. 
We evaluated predictors of the campus-wide case burden using Poisson regression models selected by backward elimination. 
Predictors specified in the full model included surface swab positivity (across 6 buildings), CO~2~ concentrations (at the time of swab collection), both on-campus and regional waste-water metrics, and Wi-Fi user counts. 

Backward elimination dropped the CO~2~ term and indicated the remaining variables as significant, positive predictors of case counts. The regional waste-water signal had the largest estimated effect, followed by peak Wi-Fi users, swab positivity, and university waste-water (Table 1).

surface swab positivity and regional waste-water measures as the most significant predictors of cases during the same period (Table S:A).

We repeated backwards elimination including interaction terms in the initial model. In the selected model, Wi-Fi (ß = 0.94; 95% CI: 0.28, 1.6), and the interaction between surface swabs and regional waste-water (ß = -0.55; 95% CI: -0.93, -0.20) were identified as significant effects, in addition to the swab positivity and regional waste-water (Table S;B). "
)
...
### TODO: probably remove this and use above instead
...
<!-- The more complex model, with 5 main effects and 7 pairwise interactions, is probably comically overfit to the limited data we have (n=29). We should probably use cross-validation to estimate model accuracy. We don't really have enough data to split into train and test sets, but we can get an estimate from cross-validation. This estimate may be somewhat pessimistically biased (less data for training per fold).   -->
...
<!-- # campus-wide cases and four predictors from stepwise selection -->
<!-- mod_df <- biweekly_data |>  -->
<!--   janitor::clean_names() |>  -->
<!--   select(-biweek, -co2) |>  -->
<!--   na.omit() -->

<!-- # 45% of intervals have zero cases -->
<!-- mean(mod_df$cases < 1) -->

<!-- poisson_recipe <-  -->
<!--   recipe(cases ~ swab_pcr + wi_fi + university_ww + ottawa_ww, -->
<!--          data = mod_df) |>  -->
...
<!-- poisson_wf <-  -->
<!--   workflow() |>  -->
<!--   add_model(poisson_reg(mode = 'regression', engine = 'glm')) |>  -->
<!--   add_recipe(poisson_recipe) -->

<!-- # fitting one model works  -->
<!-- fit(poisson_wf, data = mod_df) -->

<!-- # 10x10-fold cv to evaluate model -->
<!-- folds <- rsample::vfold_cv(mod_df, v = 10, repeats = 10) -->
<!-- fit_rs <- fit_resamples( -->
<!--   object = poisson_wf,  -->
<!--   resamples = folds,  -->
<!--   control = control_resamples(verbose = F, save_workflow = F) -->
<!--   ) -->
<!-- poisson_cv <- collect_metrics(fit_rs) -->

<!-- rmarkdown::paged_table(poisson_cv) -->


<!-- ## compare to swabs alone -->
<!-- poisson_rec_swab <-  -->
<!--   recipe(cases ~ swab_pcr, data = mod_df) |>  -->
<!--   step_normalize(all_predictors()) -->
...
<!--   workflow() |>  -->
<!--   add_model(poisson_reg(mode = 'regression', engine = 'glm')) |>  -->
<!--   add_recipe(poisson_rec_swab) -->

<!-- # 10x10-fold cv to evaluate model -->
<!-- fit_rs_swab <- poisson_wf_swab |> fit_resamples(folds) -->
<!-- poisson_cv_swab <- collect_metrics(fit_rs_swab) -->
<!-- rmarkdown::paged_table(poisson_cv_swab) -->
<!-- ``` -->
