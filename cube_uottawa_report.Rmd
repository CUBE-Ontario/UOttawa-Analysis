...
plot_site_wifi <- function(data, site){
  ...
  p <- data |> 
    ...
    scale_y_wifi() +
    rcartocolor::scale_fill_carto_d(palette = 'Safe') +
    labs(y = 'Peak Wifi<br>Connections<br>(n)', x = NULL, fill = 'Event') +
    theme_axes_lb() +
    theme(
...
ggsave('fig/figure_3.pdf', figure_3, device = 'pdf', dpi = 300, height = 6, width = 6)
...
R1 asks for further analysis of PCR tests in terms of copy numbers... specifically, to assess the correlation between floor-swab viral  load and other variables.

The results of qPCR assays for floor swabs can be analysed as a binary variable (indicating presence/absence) or numeric variable (representing viral copies) after transformation of Cq values using a standard curve. 

When viral RNA is abundant in the environment, swab positivity becomes a less useful metric as it can often reach 100%. However, floor swab positivity was generally very low during our study of the university, and never reached 100% during a single time-point. The correlation between campus-wide swab positivity and geometric mean copy number was very high (r = 0.98, CI: 0.97-0.99) and the pairwise correlations between these floor-swab statistics and other variables are nearly identical those between positivity and the same variables.

Since PCR positivity is a simple percentage, it is easier to for our reader to understand than the geometric mean number of copies *plus one*.
