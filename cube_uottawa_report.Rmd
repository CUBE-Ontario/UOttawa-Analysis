...
<!-- plt_co2 <-  -->
<!--   plt_swabs |>  -->
<!--   select(date_swab, site, traffic, co2) |>  -->
<!--   arrange(date_swab) |>  -->
<!--   glimpse() -->

<!-- plt_co2_early <- plt_co2 |> filter(date_swab < '2022-01-01') -->
<!-- plt_co2_late <- plt_co2 |> filter(date_swab > '2022-01-01') -->
...
<!--       group = traffic) -->
<!-- ) + -->
<!--   geom_line(data = plt_co2_early) + -->
<!--   geom_line(data = plt_co2_late) + -->
<!--   geom_point(data = plt_co2_early, size = 0.5) + -->
<!--   geom_point(data = plt_co2_late, size = 0.5) + -->

<!--   facet_wrap(~site, ncol = 1) + -->
<!--   scale_x_study_dates() + -->
<!--   theme_remove_grid() +  -->
<!--   theme(axis.title.y = element_markdown()) + -->
<!--   labs(x = NULL, y = "CO~2~", color = "Traffic level") -->
...
## Figure 2. (like Fig1 but disaggregated)
...
plot_site_swabs <- function(data, site){
  ...
  p <- data_early |>
    ...
    scale_y_copies() +
    scale_color_traffic() +
    labs(x = NULL, y = 'Copies + 1', color = 'Location Traffic level') +
    theme_no_x_labels() +
    theme_axes_lb() +
    theme(axis.title.y = element_markdown(),
...
# combine nested df
fig2_df_nest <- fig2_cases |> 
  left_join(fig2_swabs, by = join_by(site)) |> 
  left_join(fig2_wifi, by = join_by(site)) |> 
  left_join(fig2_co2, by = join_by(site))

## map data to plots, combine into panel for each site ====
fig2_plts <- fig2_df_nest |>
  arrange(site == '90U') |> 
  transmute(
    site,
  ...
  mutate(panel = pmap(
    ...
    .f = function(site,plt_cases, plt_swabs, plt_co2, plt_wifi) {
      p <- 
        ...
        patchwork::plot_annotation(subtitle = site)
...
```{r}
figure_1A
ggsave('fig/figure_1.png', device = 'png', width = 7, height = 7)

figure_1A
ggsave('fig/test.pdf', device = 'pdf', width = 7, height = 7)
```
