...
ggplot2::theme_set(theme_bw() + theme(
  legend.text = element_markdown(colour = 'gray30'),
  axis.text = element_markdown( colour = 'gray30'),
  axis.title = element_markdown(colour = 'gray30'),
))
...
convert_cq_to_copies <- \(cq) {
  10 ** ((40.1 - cq) / 3.23)
} 

cq_to_copies_plus_one <- \(cq){
  ifelse(
    is.na(cq),  
    1,  
    1 + (10 ** ((40.1 - cq) / 3.23))
...
swabs_tidy <- 
  ...
  mutate(
    copies_plus_one = cq_to_copies_plus_one(pcr_ct),
...
# 95% CI on the sample mean
confidence_interval <- function(x, level = 0.95) {
  if (length(x) < 2) return(
    tibble(lower = NA_real_, upper = NA_real_)
  )
  vec_sd <- sd(x, na.rm = T)
  n <- length(x[!is.na(x)])
  error <- qt((level + 1)/2, df = n - 1) *
    vec_sd / sqrt(n)
  tibble(
    lower = mean(x, na.rm = T) - error,
    upper = mean(x, na.rm = T) + error
  )
}
...
# summaries copies with geometric mean and CI
copies_summary <- function(swabs_tidy){
  swabs_tidy |>
    mutate(log_copies = log(copies_plus_one)) |> 
    summarise(
      logmean = mean(log_copies),
      logmean_ci = 
        confidence_interval(log_copies) |> 
        rename_with(~paste0('logmean_', .))
    ) |> 
    unnest(logmean_ci) |> 
    mutate(across(starts_with('logmean_'), exp)) |> 
    rename_with(~str_replace(., 'logmean', 'geomean_copiesP1'))
}
...
swabs_biweekly <- 
  ...
  positivity_summary() |> 
  left_join(
    swabs_tidy |> group_by(biweek) |> co2_summary(),
    by = join_by(biweek, n)) |> 
  left_join(
    swabs_tidy |> group_by(biweek) |> copies_summary(),
    by = join_by(biweek)
  )
...
corr_r <- corrtest$r |> signif(2)
corr_p <- corrtest$p |> signif(2)
...
fig1_cases <-
  ...
  labs(fill = NULL, title = 'UOttawa COVID-19 Cases (n)') +
  scale_fill_carto_d() +
  scale_x_study_dates() +
  theme_no_x_labels() +
  theme_remove_grid() +
...
fig1_wifi <-
  ...
  geom_point(color = theme_color,  alpha = 0.85, shape = 1, size = 1,
             na.rm = T) +
  labs(title = "Wi-Fi Connections (n)") +
  scale_x_study_dates() +
  theme_no_x_labels() +
  theme_remove_grid()
...
fig1_ott_ww <- 
  ...
  geom_point(data = regional_ww_biweekly,
             ...
             color = theme_color) +
  labs(x = 'Date',
       title = 'Ottawa Waste-Water (SARS-CoV-2:PPMoV ratio)',
       ) +
  scale_x_study_dates() +
  theme_remove_grid() +
...
figure_2 <- corrr::autoplot(corr_spearman,
                            triangular = "lower",
                            barheight = 10) +
...
plot_site_cases <- function(data, site){
  p <- data |>
    ...
    scale_fill_carto_d(
      ...
      drop = FALSE,
      name = 'Cases: Role') +
    labs(y = 'Cases (n)', x = NULL, title = site) +
    theme_no_x_labels() +
...
figure_3 <- wrap_elements(panel = h_patch) +
  labs(tag = "Date 2021-2022") +
  theme(
...
**Figure 1**: Time-series of (top to bottom) important events at the university, proportion of PCR-positive swabs, biweekly mean ambient CO~2~ (across collection sites), biweekly mean peak number of Wi-Fi connections (across 5 buildings), biweekly mean waste-water signal (across up to 6 collection sites; relative to PPMoV), biweekly mean Ottawa waste-water detection (relative to PPMoV). Points show biweekly means. Trend lines show the LOESS fit.
...
ggsave('fig/figure_1.pdf', (figure_1), device = 'pdf', dpi = 300, width = 7, height = 7)
ggsave('fig/figure_2.pdf', figure_2, device = 'pdf', dpi = 300, height = 4, width = 4)
ggsave('fig/figure_3.pdf', figure_3, device = 'pdf', dpi = 300, height = 6.5, width = 6)
...
R1 asks for further analysis of PCR tests in terms of copy numbers... specifically, to assess the correlations between floor-swab viral  load and other variables. The results of qPCR assays for floor swabs can be analysed as a binary variable (indicating presence/absence) or numeric variable (representing viral copies) after transformation of Cq values using a standard curve. Groups of test results can be summarised as either a positivity rate or an average copy number (we usually use a geometric mean).

When viral RNA is abundant in the environment, swab positivity becomes a less useful metric as it can often reach 100%. However, floor swab positivity was generally low during our study of the university, and never reached 100% at any time-point. The correlation between campus-wide swab positivity and geometric mean copy number was very high (r = 0.98, CI: 0.97-0.99) and the pairwise correlations between these floor-swab statistics and other variables are nearly identical to those between positivity and the same variables (e.g. correlation with cases is r = 0.746 for copies and r =  0.743 for positivity).

```{r}
corr_ci |> 
  mutate(
    terms = terms |> 
      str_replace('SwPCR', 'Positivity') |> 
      str_replace('Swbcp', 'Copies')
  ) |> 
  filter(str_detect(terms, 'Copies|Pos')) |> 
  mutate(across(where(is.numeric), ~signif(., digits = 2))) |> 
  gt::gt(caption = 'Spearman correlation coefficients are almost identical for viral positivity and copies')
