...
study_period_weeks <- crossing(
  ...
  site = unique(per_bldg_cases$site)
  ) |> 
  mutate(
    week = get_date_week(date),
    redate = week_to_date(week)) |> view()
  distinct(site, week)
...
per_bldg_cases <- 
  ...
  right_join(study_period_weeks, by = join_by(week, site)) |> 
  mutate(across(where(is.integer), ~if_else(is.na(.), 0, .))) |> 
  arrange(week, site) |> 
...
**Poisson GLM: Cases ~ positivity + (1|site)**
...
```{r}
broom.mixed::tidy(per_bldg_model, 'ran_vals',  conf.int = T, conf.level = 0.95, exponentiate = T) |> 
  ggplot(aes(y = level,
