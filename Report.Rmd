...
ottawa_u_mean_signal <- 
  uottawa_ww |> 
  group_by(sample_date) |> 
  summarise(
    mean_signal = mean(viral_copies_per_copies_pep_avg),
    sd = sd(viral_copies_per_copies_pep_avg)
    )

uottawa_ww |> 
  ggplot(aes(sample_date, viral_copies_per_copies_pep_avg)) +
  geom_point(alpha = 0.5, shape = 1) +
  geom_point(data = ottawa_u_mean_signal,
             aes(y = mean_signal),
             alpha = 0.8,
             color = 'cornflowerblue') +
  geom_path(data = ottawa_u_mean_signal,
            aes(y = mean_signal),
            alpha = 0.8,
            color = 'cornflowerblue') +
  labs(x = 'Date',
       y = 'SARS-CoV-2 / PPMoV',
       title = 'UOttawa Waste-Water Signal',
       subtitle = 'Daily mean across sampling sites'
       )
