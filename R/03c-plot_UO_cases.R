...
plt_sites <-
  ...
  scale_x_date(limits = c(min(cube$date_swab) - 5,
                          max(cube$date_swab) + 1)) +
...
plt_cases_swabs <-
  ...
  # plot_annotation(
    # caption = "Black rug lines indicate positive test dates for each case.\n Red rug lines show the assumed start of transmissiblity for each case."
    # )
